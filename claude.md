...
4. **プログラミングレッスン**: ステップバイステップのPython学習
5. **プログラミング問題演習**: AtCoder風のコード判定システム
...
### 新しい問題セットを追加する際の必須手順（重要！⚠️）

**問題演習システムに新しい問題セットを追加する際は、以下の2つのファイルを必ず更新してください：**

1. **`public/questions/index.yaml`への追加**
   - `questionSets`配列に新しい問題セットのIDを追加
   - この手順を忘れると、問題データは存在しても読み込まれない

2. **`src/pages/quiz/index.astro`の`categoryConfig`への追加（最重要！）**
   - 該当するカテゴリの`order`配列に新しい問題セットのIDを追加
   - 例：冬休み課題の場合、`winter-break`カテゴリの`order`配列に追加
   - **この手順を忘れると、問題一覧ページに表示されません！**
   - 場所：378行目付近の`categoryConfig`配列

**この2つのファイルへの追加を忘れると、問題が表示されないトラブルが必ず発生します。毎回確認してください。**

### その他の注意点

- 新しい単元を追加する際は、上記のステップを実行すること（動的ルーティングは自動更新）
...
3. **解説は簡潔に（重要！⚠️）**
   - **「実行の流れ」のような詳細なステップ説明は絶対に書かない**
   - 詳細に書くと、それがそのまま解答やヒントになってしまう
   - 解説は解答を見た後に理解を深めるための簡潔な説明にとどめる
   - 例：「条件が真なので、if文の中が実行されます。」程度の簡潔さ
