...
var simcir = function($) {
  ...
  var setupSimcir = function($placeHolder, data) {
    ...
    var saveCircuit = function() {
      var circuitData = controller($workspace).text();
      var blob = new Blob([circuitData], {type: 'application/json'});
      var url = URL.createObjectURL(blob);
      var a = document.createElement('a');
      a.href = url;
      
      // Generate filename with current date and time (YYYYMMDD_HHMMSS)
      var now = new Date();
      ...
      var filename = 'circuit_' + year + month + day + '_' + hours + minutes + seconds + '.json';
        ...
        if (file) {
          ...
          reader.onload = function(e) {
            try {
              ...
              // Rebuild the workspace with loaded data
              $workspace.remove();
              $workspace = simcir.createWorkspace(loadedData);
              $('.simcir-body').prepend($workspace);
            } catch (error) {
    ...
    var saveAsImage = function() {
      ...
      // Create canvas and save as PNG - improved version
      try {
        console.log('Workspace element:', $workspace[0]);
        console.log('SVG search result:', $workspace.find('svg'));
        
        var svgElement = $workspace.find('svg')[0]; // Get the actual SVG element
        if (!svgElement) {
          // Try alternative methods to find SVG
        ...
        if (!svgElement) {
          ...
          alert('Failed to find SVG element for image export. Please make sure the circuit is displayed properly.');
          return;
        }
        ...
        // Preserve computed styles for dynamic elements
        var preserveComputedStyles = function(originalEl, clonedEl) {
          if (originalEl.nodeType === 1) { // Element node
            var computedStyle = window.getComputedStyle(originalEl);
            var styleProps = ['fill', 'stroke', 'stroke-width', 'opacity', 'visibility', 'color', 'font-family', 'font-size'];
            ...
            if (inlineStyles.length > 0) {
              var existingStyle = clonedEl.getAttribute('style') || '';
              clonedEl.setAttribute('style', existingStyle + '; ' + inlineStyles.join('; '));
            }
          }
          
          // Recursively process children
          for (var i = 0; i < originalEl.childNodes.length; i++) {
            if (clonedEl.childNodes[i]) {
              preserveComputedStyles(originalEl.childNodes[i], clonedEl.childNodes[i]);
            }
          }
        };
        
        // Apply computed styles to preserve dynamic states
        preserveComputedStyles(svgElement, svgClone);
        
        // Add CSS styles inline to the SVG
        var styleElement = document.createElementNS('http://www.w3.org/2000/svg', 'style');
        var cssRules = [];
        
        // Extract relevant CSS rules from stylesheets
        try {
          for (var i = 0; i < document.styleSheets.length; i++) {
            var styleSheet = document.styleSheets[i];
            try {
              var cssRules_temp = styleSheet.cssRules || styleSheet.rules;
              for (var j = 0; j < cssRules_temp.length; j++) {
                var rule = cssRules_temp[j];
        ...
        // Add comprehensive styling - always include to ensure colors are preserved
        var comprehensiveRules = [
          ...
          '.simcir-connector { stroke-width: 2; stroke: #0000ff; stroke-linecap: round; fill: none; }',
          '.simcir-device { fill: #cccccc; stroke-width: 2; stroke: #666666; }',
          '.simcir-device-selected { stroke: #0000ff; }',
          '.simcir-device-label { fill: #000000; stroke-width: 0; stroke: none; }',
        ...
        // Merge comprehensive rules with extracted rules, avoiding duplicates
        var allRules = comprehensiveRules.slice(); // Copy comprehensive rules
        for (var k = 0; k < cssRules.length; k++) {
          var ruleText = cssRules[k];
          var isDuplicate = false;
          for (var l = 0; l < allRules.length; l++) {
            if (allRules[l].indexOf(ruleText.split('{')[0].trim()) === 0) {
              isDuplicate = true;
              break;
            }
          }
        ...
        console.log('Applied CSS rules count:', cssRules.length);
        console.log('Sample CSS rules:', cssRules.slice(0, 5));
        ...
        // Create the image using data URL instead of blob URL to avoid CORS issues
        ...
        img.onload = function() {
          ...
          if (canvas.toBlob) {
            ...
            pngLink.click();
            document.body.removeChild(pngLink);
            console.log('PNG download initiated using fallback method');
          }
        };
        ...
        img.onerror = function(e) {
          ...
          alert('Failed to convert SVG to image. The circuit may be too complex or contain unsupported elements.');
        };
        
        // Set a timeout to detect loading issues
        setTimeout(function() {
          if (!img.complete) {
            console.warn('Image loading is taking longer than expected');
          }
        }, 5000);
        ...
        console.error('Error in saveAsImage:', error);
        alert('Failed to save image: ' + error.message);
      }
    };

    // Create control buttons
    var $controls = $('<div></div>').addClass('simcir-controls').css({
      'margin': '10px 0',
      'text-align': 'left'
    });
    var $saveBtn = $('<button>回路を保存する</button>').css({
      'margin-right': '10px',
      'padding': '5px 10px',
      ...
      'border-radius': '3px',
      'cursor': 'pointer'
    }).on('click', saveCircuit);
    var $loadBtn = $('<button>回路を読み込む</button>').css({
      'margin-right': '10px',
      'padding': '5px 10px',
      ...
      'border-radius': '3px',
      'cursor': 'pointer'
    }).on('click', loadCircuit);
    var $saveImageBtn = $('<button>回路を画像として保存する</button>').css({
      'padding': '5px 10px',
    ...
    $controls.append($saveBtn).append($loadBtn).append($saveImageBtn);
    ...
    $placeHolder.append($controls).append($body);
    toggle();
    return $placeHolder;
  };
